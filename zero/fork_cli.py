...
def _validate_liquidation_payload(payload: dict) -> tuple[dict, list, int, str]:
    candidate = payload.get('candidate')
    steps = payload.get('steps')
    if not isinstance(candidate, dict):
        raise ValueError('liquidation candidate payload is required')
    if payload.get('kind') != 'liquidation':
        raise ValueError('liquidation payload kind is required')
    if not isinstance(steps, list) or len(steps) != 4:
        raise ValueError('exactly four liquidation execution steps are required')
    block = int(candidate.get('block', 0))
    if block <= 0:
        raise ValueError('liquidation candidate block must be positive')
    asset = str(candidate.get('base_asset', ''))
    if not asset.startswith('0x') or len(asset) != 42:
        raise ValueError('liquidation debt asset must be an address')
    return candidate, steps, block, asset
...
def _liquidation_env(upstream_rpc: str, payload: dict,
    ...
    env['FORK_BLOCK'] = str(block)
    env['ZERO_ASSET'] = asset
    env['ZERO_LOAN_RAW'] = str(int(candidate['debt_to_cover_raw']))
    env['ZERO_MIN_PROFIT_RAW'] = str(int(candidate['min_profit_raw']))
    env['ZERO_RESULT_PATH'] = result_path
    for i, step in enumerate(steps):
        if int(step.get('value', 0)) != 0:
            raise ValueError('liquidation steps must have zero ETH value')
        target = str(step.get('target', ''))
        data = str(step.get('data', ''))
        if not target.startswith('0x') or len(target) != 42:
            raise ValueError(f'liquidation step {i} target must be an address')
        if not data.startswith('0x'):
            raise ValueError(f'liquidation step {i} calldata must be hex-prefixed')
        env[f'ZERO_STEP{i}_TARGET'] = target
        env[f'ZERO_STEP{i}_DATA'] = data
    return env, candidate, block


def run_live_liquidation_fork_result(upstream_rpc: str, payload: dict) -> ForkResult:
    """Replay one four-step liquidation payload and return fork economics."""
    result_path = _new_result_path()
    env, candidate, block = _liquidation_env(
        upstream_rpc, payload, result_path)
    verification = payload.get('verification') or {}
    reserve_usd = float(verification.get('model_reserve_usd', 0.0))
    predicted_gate_net = float(candidate.get('predicted_net', 0.0))
    predicted_model_net = predicted_gate_net + reserve_usd
    strategy = str(verification.get('strategy', 'swarm_liquidation'))
    completed = None
    try:
        completed = subprocess.run(
            ['bash', 'scripts/live_liquidation_fork_test.sh'],
        ...
        if completed.returncode != 0:
            detail['stdout_tail'] = (completed.stdout or '')[-2000:]
            detail['stderr_tail'] = (completed.stderr or '')[-2000:]
            return ForkResult(
                block=block, strategy=strategy, success=False, gas_used=0,
                predicted_net=predicted_model_net, realized_net=0.0,
                detail=json.dumps(detail, sort_keys=True),
                outcome_class=_classify_failed_replay(completed))
        path = Path(result_path)
        if not path.exists():
            detail['error'] = 'missing_result_file'
            return ForkResult(
                block=block, strategy=strategy, success=False, gas_used=0,
                predicted_net=predicted_model_net, realized_net=0.0,
                detail=json.dumps(detail, sort_keys=True),
        ...
        decimals = int(candidate['base_decimals'])
        base_price_usd = float(verification.get('base_price_usd', 1.0))
        realized_profit_usd = realized_raw / (10 ** decimals) * base_price_usd
        modeled_gas_usd = float(candidate.get('gas_cost_usd', 0.0))
        gas_limit = int(verification.get('gas_limit', 0) or 0)
        estimated_gas_cost_usd = (
            modeled_gas_usd * gas_used / gas_limit
            if gas_limit > 0 else modeled_gas_usd)
        realized_net_usd = realized_profit_usd - estimated_gas_cost_usd
        detail.update({
            'realized_raw': str(realized_raw),
            'realized_profit_usd': realized_profit_usd,
        ...
        return ForkResult(
            block=block, strategy=strategy, success=True,
            gas_used=gas_used, predicted_net=predicted_model_net,
            realized_net=realized_net_usd,
            detail=json.dumps(detail, sort_keys=True))
    except Exception as exc:
        detail = {
